...
const
  ...
  // 需要展示给用户的设备名称列表
  deviceNameSet = new Set(["HC-08", "BT05", "MLT-BT05", "XWJv2", "XWJv3"]),
  //是否隐藏除吸污机以外的其他设备
  ...
  acceptDataValid = (arr) => {
    ...
    if (arr[13] !== 0 || arr[13] !== 1) {
      flag = 13;
      msg = `开泵和爬墙状态0 1不为${arr[13]}`
    }
    if (arr[13] !== 255 && arr[13] > 100) {
      flag = 14;
      msg = `超声波距离0~100 255不为${arr[13]}`
    }
    return {
      success: flag === -2,
      errorIndex: flag,
  ...
  isAcceptDataValid = (arr) => {
    ...
    if (arr[13] !== 0 || arr[13] !== 1) return false;
    if (arr[13] !== 255 && arr[13] > 100) return false;
    return true
