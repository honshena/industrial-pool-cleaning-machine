...
const {
  BluetoothManager
} = require("../../v3/utils/BluetoothController.js")
const {
  deviceNameSet
...
Page({
  ...
  async isBuletoothAndLocationPrepared() {
    ...
    return new Promise(async (resolve, reject) => {
      const prepared = await buletoothManager.isBuletoothAndLocationPrepared()
      //位置信息和蓝牙未正常打开
  ...
  async connect(e) {
    ...
    // 新设备连接成功后会自动跳转至新页面
    if (name === "XWJv3") {
      logWarn(`检测到用户使用新设备[${name}]即将跳转至[${PAGE_VERSION[PAGE_KEYS.lastest].path}]`, {
        name,
        deviceName
      })
      this.releaseBluetooth();
      changePageVersion(PAGE_KEYS.lastest);
      return;
    }
