...
Page({
    ...
    async onShow() {
        ...
        if (version === version_develop && useTest) {
            let count = 0,
                // 0保留 1:  2:机型 13: 开泵和爬墙 14: 超声波
                arr = [0, 1, 3, 2, 4, 3, 6, 7, 8, 9, 10, 11, 8, 1, 25]
    ...
    resolvePackage(arr = []) {
        ...
        */
         ...
         * 11: 显示超声波位
         */
        const input = [0, arr[3], arr[8], arr[4], this.newInput?.[4], arr[2], arr[12], arr[5], arr[7], arr[6], arr[13], arr[14]];
        // 是否需要更新input,当第一次校验失败时不更新
        let needUpdateInput = true;
        // 检查设置的是否生效
        if (this.needSettingCheck) {
            // console.log('更改设置: ','input:',input[6],'newInput: ',this.newInput[6],'oldSetting:',this.oldSetting.get(6))
            // logInfo('更改设置: ','input:',input,'newInput: ',this.newInput,'oldSetting:',[...this.oldSetting])
            if (this.checkSettingIsActive(input)) {
                // 两次内检查到成功
                this.settingCheckErrorCount = 0;
                this.needSettingCheck = false;
                this.clearShowErrorTimeout();
                // showToast('更改设置成功');
            } else if (this.settingCheckErrorCount > this.MAX_ALLOW_ERROR_PACKAGE) {
                // 超过两次检查失败
                this.settingCheckErrorCount = 0;
                this.needSettingCheck = false;
                this.clearShowErrorTimeout();
    ...
    oldSetting: null, //这里不初始化避免出问题 onLoad初始化 new Map(), //旧的设置,需要与新的对比, key为input的索引,value为input的值
    showErrorTimeout: null,
    MAX_ALLOW_ERROR_PACKAGE: 2,
    clearShowErrorTimeout() {
    ...
    setShowErrorTimeout() {
        ...
        this.showErrorTimeout = setTimeout(() => {
            showToast('更改设置失败');
        }, acceptMAXInterval * this.MAX_ALLOW_ERROR_PACKAGE)
    },
    copyPhone() {
        // wx.setClipboardData({
        //     data: '13213560009',
        ...
        // })
        wx.makePhoneCall({
            phoneNumber: '13213560009' //仅为示例，并非真实的电话号码
          })
    },
    ...
    async connect(e) {
        ...
        // 老设备连接成功后会自动跳转至老页面
        if (!(name === "XWJv3")) {
            logWarn(`检测到用户使用老设备[${name}]即将跳转至[${PAGE_VERSION[PAGE_KEYS.v3].path}]`, {
                name,
                deviceName
            });
            this.releaseBluetooth();
            changePageVersion(PAGE_KEYS.v3);
            return;
        }
        const isConnect = await this.connectBLEDevice(lastestConnecteddevice, false) //连接新设备
