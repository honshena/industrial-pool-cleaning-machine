...
const { deviceNameSet } = require('../../config/bluetooth')
const {
    hideLoading,
    showLoading,
    showToast,
...
class BluetoothManager {
    ...
    initConnect(device = {}, reconnect = false, BLEStateChangeCallback, time = sendInterval0) {
        const self = this
        return new Promise(async (resolve, reject) => {
            ...
            self.characteristicId = config.characteristicId
            logInfo(`${reconnect?'重连上次设备':'连接新设备' }名称: [${self.deviceName}], 设备显示名称: [${self.name}], 设备MAC地址: [${self.deviceId}], 设备服务号: [${self.serviceId}], 设备特征值: [${self.characteristicId}]`)
            ...
            if (isConnected.res) {
                ...
                //设置上次连接设备
                logWarn(`连接设备[${self.deviceName}]成功, 设备显示名称: ${self.name}, 设备本地名称 ${self.deviceLocalName}, 设备MAC地址: ${self.deviceId}, 设备服务号: ${self.serviceId}, 设备特征值: ${self.characteristicId},设备详细信息: `, deviceInfo, '连接结果: ', isConnected)

            } else {
                logError(`连接设备[${self.deviceName}]失败, 设备显示名称: ${self.name},  设备本地名称 ${self.deviceLocalName}, 设备MAC地址: ${self.deviceId}, 设备服务号: ${self.serviceId}, 设备特征值: ${self.characteristicId},连接失败信息: `, isConnected)
            ...
            //成功获取到服务号和特征值
            logInfo(`成功获取服务号: [${this.serviceId}] 和其对应特征值: [${serviceAndCharacteristic.info.characteristics.characteristics[0].uuid}]`, serviceAndCharacteristic)
    ...
    isBluetoothAdapterOpen() {
        return new Promise((resolve, reject) => {
            wx.openBluetoothAdapter({
                ...
                fail: function (err) {
                    return resolve(resAndInfo(false, err));
                }
            })
        })
    }
    ...
    startSearchBluetoothDeviceAround() {
        ...
        return new Promise((resolve, reject) => {
            if (self.isStartBluetoothDevicesDiscovery)
                return resolve(resAndInfo(true, {}, '已开始搜索附近蓝牙设备'))
    ...
    startUpdateSearchBluetoothDeviceAround(callback, ishideOtherDevices) {
        ...
        wx.onBluetoothDeviceFound(function (e) {
            ...
            if (ishideOtherDevices) {
                //隐藏其他设备
                if (deviceNameSet.has(e.devices[0].name)) {
                    //console.log('是吸污机',e);
    ...
    //连接蓝牙
    connectBLE(deviceId, timeout = 5000) {
        const self = this
        return new Promise((resolve, reject) => {
    ...
    //发送单个数据
    writeBLECharacteristicValue(data, deviceId, serviceId, characteristicId) {
        const self = this
        return new Promise((resolve, reject) => {
            ...
            wx.writeBLECharacteristicValue({
                deviceId: deviceId || self.deviceId,
                serviceId: serviceId || self.serviceId,
                characteristicId: characteristicId || self.characteristicId,
